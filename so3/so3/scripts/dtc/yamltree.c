...
char *yaml_error_name[] = {
	[YAML_NO_ERROR] = "no error",	      [YAML_MEMORY_ERROR] = "memory error",
	[YAML_READER_ERROR] = "reader error", [YAML_SCANNER_ERROR] = "scanner error",
	[YAML_PARSER_ERROR] = "parser error", [YAML_COMPOSER_ERROR] = "composer error",
	[YAML_WRITER_ERROR] = "writer error", [YAML_EMITTER_ERROR] = "emitter error",
};

#define yaml_emitter_emit_or_die(emitter, event)                                                                               \
	({                                                                                                                     \
		if (!yaml_emitter_emit(emitter, event))                                                                        \
			die("yaml '%s': %s in %s, line %i\n", yaml_error_name[(emitter)->error], (emitter)->problem, __func__, \
			    __LINE__);                                                                                         \
	})
...
static void yaml_propval_int(yaml_emitter_t *emitter, struct marker *markers, char *data, int len, int width)
{
	...
	yaml_sequence_start_event_initialize(&event, NULL, (yaml_char_t *) tag, width == 4, YAML_FLOW_SEQUENCE_STYLE);
	...
	for (off = 0; off < len; off += width) {
		...
		if (is_phandle)
			yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *) "!phandle", (yaml_char_t *) buf, strlen(buf),
						     0, 0, YAML_PLAIN_SCALAR_STYLE);
		else
			yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *) YAML_INT_TAG, (yaml_char_t *) buf,
...
static void yaml_propval_string(yaml_emitter_t *emitter, char *str, int len)
{
	...
	yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *) YAML_STR_TAG, (yaml_char_t *) str, len - 1, 0, 1,
...
static void yaml_propval(yaml_emitter_t *emitter, struct property *prop)
{
	...
	yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *) YAML_STR_TAG, (yaml_char_t *) prop->name, strlen(prop->name),
				     1, 1, YAML_PLAIN_SCALAR_STYLE);
	yaml_emitter_emit_or_die(emitter, &event);

	/* Boolean properties are easiest to deal with. Length is zero, so just emit 'true' */
	if (len == 0) {
		yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *) YAML_BOOL_TAG, (yaml_char_t *) "true",
	...
	yaml_sequence_start_event_initialize(&event, NULL, (yaml_char_t *) YAML_SEQ_TAG, 1, YAML_FLOW_SEQUENCE_STYLE);
	yaml_emitter_emit_or_die(emitter, &event);
...
static void yaml_tree(struct node *tree, yaml_emitter_t *emitter)
{
	...
	for_each_property(tree, prop) yaml_propval(emitter, prop);

	/* Loop over all the children, emitting them into the map */
	for_each_child(tree, child)
	{
		yaml_scalar_event_initialize(&event, NULL, (yaml_char_t *) YAML_STR_TAG, (yaml_char_t *) child->name,
