...
static const char *processor_modes[] = { "USER_26", "FIQ_26", "IRQ_26",	 "SVC_26",  "UK4_26",  "UK5_26",  "UK6_26",  "UK7_26",
...
void show_registers(struct cpu_regs *regs)
{
	...
	printk("pc : [<%08lx>]    lr : [<%08lx>]    \n"
	       ...
	       (unsigned long) regs->pc, (unsigned long) regs->lr, (unsigned long) regs->sp, (unsigned long) regs->ip,
	...
	printk("Flags: %c%c%c%c", flags & PSR_N_BIT ? 'N' : 'n', flags & PSR_Z_BIT ? 'Z' : 'z', flags & PSR_C_BIT ? 'C' : 'c',
	       flags & PSR_V_BIT ? 'V' : 'v');
	printk("  IRQs o%s  FIQs o%s  Mode %s%s\n", interrupts_enabled(regs) ? "n" : "ff",
	       fast_interrupts_enabled(regs) ? "n" : "ff", processor_modes[processor_mode(regs)], "ARM");
