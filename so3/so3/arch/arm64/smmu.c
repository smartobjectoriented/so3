 ...
 * Copyright 2023 REDS HEIG-VD, Daniel Rossier
 *
...
void arm_smmu_init(struct domain *d)
{
	struct iommu iommu;
	struct arm_smmu_device *smmu;
	int err;

	iommu.size = 0x00020000;

	iommu.base = io_map(0x09050000, iommu.size);
	BUG_ON(!iommu.base);
	...
	arm_smmu_device_init_features(smmu);
