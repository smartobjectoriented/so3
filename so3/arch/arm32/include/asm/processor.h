...
#define __ERET __inst_arm32(0xE160006E, 0xF3DE8F00)

#define __MSR_ELR_HYP(regnum) \
	__inst_arm32(0xE12EF300 | regnum, 0xF3808E30 | (regnum << 16))

#define __SMC(imm4)                                      \
	__inst_arm32(0xE1600070 | (((imm4) & 0xF) << 0), \
...
#define interrupts_enabled(regs) (!((regs)->psr & PSR_I_BIT))

#define fast_interrupts_enabled(regs) (!((regs)->psr & PSR_F_BIT))

#define processor_mode(regs) ((regs)->psr & PSR_MODE_MASK)
...
static inline int cpu_mode(void)
{
	...
	asm volatile("mrs     %0, cpsr" : "=r"(cpsr) : : "memory", "cc");
...
{
	asm volatile("msr	" IRQMASK_REG_NAME_W
		     ", %0	@ local_irq_restore"
		     :
		     : "r"(flags)
		     : "memory", "cc");
