...
void arch_prepare_cpu_regs(tcb_t *tcb, clone_args_t *args)
{
	...
	if (args->pcb == NULL) {
		...
		BUG_ON(args->fn == NULL);
		tcb->cpu_regs.x19 = (unsigned long) args->fn;
		tcb->cpu_regs.x20 = (unsigned long) args->fn_arg;
...
void arch_restart_user_thread(tcb_t *tcb, th_fn_t fn_entry, addr_t stack_top)
{
	 ...
	 * which needs to be set to the properly start the program
	 */
...
{
	return get_kernel_stack_top(tcb->stack_slotID) - S_FRAME_SIZE;
}
