 ...
 * \return
 */
ROXML_STATIC ROXML_INT void roxml_del_from_pool(node_t *root, node_t *n,
						int req_id)
{
	roxml_lock(root);
	if (n->priv) {
 ...
 */
ROXML_STATIC ROXML_INT int roxml_add_to_pool(node_t *root, node_t *n,
					     int req_id)
{
	xpath_tok_t *tok;
 ...
 * \return
 */
ROXML_STATIC ROXML_INT void roxml_release_id(node_t *root, node_t **pool,
					     int pool_len, int req_id)
{
	int i;
 ...
 */
ROXML_STATIC ROXML_INT void roxml_compute_and(node_t *root, node_t **node_set,
					      int *count, int cur_req_id,
					      int prev_req_id)
{
	int i = 0;
	int limit = *count;
 ...
 * \return
 */
ROXML_STATIC ROXML_INT void roxml_compute_or(node_t *root, node_t **node_set,
 ...
 */
ROXML_STATIC ROXML_INT int roxml_parse_xpath(char *path, xpath_node_t **xpath,
					     int context)
{
	int ret = 0;
	...
	parser =
		roxml_append_parser_item(parser, ">", _func_xpath_operator_sup);
	parser =
		roxml_append_parser_item(parser, "<", _func_xpath_operator_inf);
	parser = roxml_append_parser_item(parser, "!",
	...
	parser =
		roxml_append_parser_item(parser, "+", _func_xpath_operator_add);
	parser = roxml_append_parser_item(parser, "-",
					  _func_xpath_operator_subs);
	parser = roxml_append_parser_item(parser, ROXML_PATH_OR,
					  _func_xpath_path_or);
	parser = roxml_append_parser_item(parser, ROXML_COND_OR,
	...
	parser = roxml_append_parser_item(parser, ROXML_FUNC_FIRST_STR,
					  _func_xpath_first);
	parser = roxml_append_parser_item(parser, ROXML_FUNC_LAST_STR,
					  _func_xpath_last);
	parser = roxml_append_parser_item(parser, ROXML_FUNC_NSURI_STR,
					  _func_xpath_nsuri);
	parser = roxml_append_parser_item(parser, ROXML_FUNC_LNAME_STR,
					  _func_xpath_lname);
 ...
 */
ROXML_STATIC ROXML_INT int roxml_validate_predicat(xpath_node_t *xn,
						   ...
						   node_t *candidat)
{
	...
	while (condition) {
		...
		case ROXML_FUNC_FIRST:
			if (condition->op > 0)
				iarg2 = roxml_double_oper(
					iarg2,
					roxml_strtonum(condition->arg2, NULL),
					condition->op);
			status = roxml_double_cmp(iarg1, iarg2,
						  ROXML_OPERATOR_EQU);
			break;
		case ROXML_FUNC_INTCOMP:
			if (condition->arg1)
				val = roxml_get_attr(candidat,
						     condition->arg1 + 1, 0);
		...
		case ROXML_FUNC_NSURI:
			...
			sarg1 = roxml_get_content(val, NULL, 0, &status);
			status = roxml_string_cmp(sarg1, condition->arg2,
						  condition->op);
			roxml_release(sarg1);
			break;
		...
		case ROXML_FUNC_XPATH:
			...
			node_set = roxml_exec_xpath(val, candidat,
						    condition->xp,
						    condition->func2, &status);
...
ROXML_STATIC ROXML_INT void roxml_add_to_set(node_t *root, node_t *candidat,
					     node_t ***ans, int *nb, int *max,
					     int req_id)
{
	if (roxml_add_to_pool(root, candidat, req_id)) {
		if (ans) {
			if ((*nb) >= (*max)) {
				int new_max = (*max) * 2;
				node_t **new_ans =
					roxml_malloc(sizeof(node_t *), new_max,
						     PTR_NODE_RESULT);
				memcpy(new_ans, (*ans),
				       *(max) * sizeof(node_t *));
				roxml_release(*ans);
...
ROXML_STATIC ROXML_INT int
roxml_validate_axe_func(node_t *root, node_t **candidat, xpath_node_t *xn)
{
	int valid = 0;
	char *axes = xn->name;
						   ...
						   xpath_node_t *xn)
{
	...
	if (candidat->ns) {
		name = roxml_get_name(candidat->ns, intern_buff,
				      ROXML_BASE_LEN);
		ns_len = strlen(name);
		if (ns_len)
			name[ns_len++] = ':';
 ...
 */
ROXML_STATIC ROXML_INT int roxml_validate_axes(node_t *root, node_t *candidat,
					       node_t ***ans, int *nb, int *max,
					       xpath_node_t *xn, int req_id)
{
	...
	if (xn == NULL) {
		...
		if (xn->next == NULL)
			path_end = 1;
		if ((xn->axes == ROXML_ID_SELF) ||
		    (xn->axes == ROXML_ID_PARENT))
			valid = 1;
 ...
 * \return
 */
ROXML_STATIC ROXML_INT void roxml_check_node(xpath_node_t *xp, node_t *root,
					     node_t *context, node_t ***ans,
					     int *nb, int *max, int ignore,
					     int req_id)
{
	int validate_node = 0;
	...
	if (ignore == ROXML_DESC_ONLY) {
		...
		while (current) {
			roxml_check_node(xp, root, current, ans, nb, max,
					 ignore, req_id);
	...
	case ROXML_ID_CHILD: {
		node_t *current = context->chld;
		while (current) {
			validate_node = roxml_validate_axes(
				root, current, ans, nb, max, xp, req_id);
			if (validate_node)
				roxml_check_node(xp->next, root, current, ans,
						 nb, max, ROXML_DIRECT, req_id);
			current = current->sibl;
		}
	...
	case ROXML_ID_DESC: {
		roxml_check_node(xp->next, root, context, ans, nb, max,
				 ROXML_DESC_ONLY, req_id);
	} break;
	case ROXML_ID_DESC_O_SELF: {
		xp = xp->next;
		validate_node = roxml_validate_axes(root, context, ans, nb, max,
						    xp, req_id);
		if (validate_node)
			roxml_check_node(xp->next, root, context, ans, nb, max,
					 ROXML_DIRECT, req_id);
		roxml_check_node(xp, root, context, ans, nb, max,
				 ROXML_DESC_ONLY, req_id);
	} break;
	case ROXML_ID_SELF: {
		validate_node = roxml_validate_axes(root, context, ans, nb, max,
	...
	case ROXML_ID_PARENT: {
		if (context->prnt) {
			validate_node = roxml_validate_axes(
				root, context->prnt, ans, nb, max, xp, req_id);
			roxml_check_node(xp->next, root, context->prnt, ans, nb,
					 max, ROXML_DIRECT, req_id);
		} else {
			validate_node = 0;
		}
	} break;
	case ROXML_ID_ATTR: {
		...
		while (attribute) {
			validate_node = roxml_validate_axes(
				root, attribute, ans, nb, max, xp, req_id);
			if (validate_node)
				roxml_check_node(xp->next, root, context, ans,
						 nb, max, ROXML_DIRECT, req_id);
	...
	case ROXML_ID_NEXT: {
		...
		while (current) {
			...
			while (following) {
				validate_node = roxml_validate_axes(
					root, following, ans, nb, max, xp,
					req_id);
				if (validate_node) {
					roxml_check_node(xp->next, root,
							 following, ans, nb,
							 max, ROXML_DIRECT,
							 req_id);
				} else {
					xp->axes = ROXML_ID_CHILD;
					roxml_check_node(xp, root, following,
							 ans, nb, max,
							 ROXML_DESC_ONLY,
							 req_id);
		...
		while (current && current->prnt) {
			...
			while (preceding != current) {
				...
				if (validate_node) {
					...
					xp->axes = ROXML_ID_CHILD;
					roxml_check_node(xp, root, preceding,
							 ans, nb, max,
							 ROXML_DESC_ONLY,
							 req_id);
	...
	case ROXML_ID_NS: {
		validate_node = roxml_validate_axes(root, context->ns, ans, nb,
						    max, xp, req_id);
		if (validate_node)
			roxml_check_node(xp->next, root, context, ans, nb, max,
					 ROXML_DIRECT, req_id);
...
ROXML_INT node_t **roxml_exec_xpath(node_t *root, node_t *n,
	...
	*count = 0;
	...
	for (path_id = 0; path_id < index; path_id++) {
		...
		if ((cur_xpath->rel == ROXML_OPERATOR_AND) ||
			...
			roxml_check_node(cur_xpath, root, orig, &node_set,
					 count, &max_answers, ROXML_DIRECT,
					 req_id);

			if (cur_xpath->rel == ROXML_OPERATOR_AND)
				roxml_compute_and(root, node_set, count, req_id,
						  req_ids[path_id - 1]);
	...
	for (path_id = 0; path_id < index; path_id++) {
		...
		if (cur_xpath->rel == ROXML_OPERATOR_OR) {
			...
			if (req_ids[path_id] == 0) {
				...
				roxml_check_node(cur_xpath, root, orig,
						 &node_set, count, &max_answers,
						 ROXML_DIRECT, glob_id);
			} else {
				roxml_compute_or(root, node_set, count,
						 req_ids[path_id + 1], glob_id);
				roxml_release_id(root, node_set, *count,
						 req_ids[path_id + 1]);
			}
		}
	}
 ...
 */
ROXML_STATIC ROXML_INT xpath_node_t *roxml_set_axes(xpath_node_t *node,
						    char *axes, int *offset)
{
	struct _xpath_axes {
		char id;
		char *name;
	...
	if (axes[0] == '/') {
	...
	} else if (strncmp(ROXML_L_DESC_O_SELF, axes,
			   strlen(ROXML_L_DESC_O_SELF)) == 0) {
		/* ROXML_L_DESC_O_SELF */
		node->axes = ROXML_ID_DESC_O_SELF;
		...
		*offset += strlen(ROXML_L_DESC_O_SELF);
		...
		node->next = tmp_node;
		node = roxml_set_axes(
			tmp_node, axes + strlen(ROXML_L_DESC_O_SELF), offset);
		...
		*offset += strlen(ROXML_L_DESC);
		...
		tmp_node->axes = ROXML_ID_CHILD;
		node->next = tmp_node;
		node = roxml_set_axes(tmp_node, axes + strlen(ROXML_L_DESC),
				      offset);
	} else {
		int i = 0;
...
ROXML_INT int _func_xpath_ignore(roxml_parser_item_t *parser, char *chunk,
				 void *data)
{
#ifdef DEBUG_PARSING
...
ROXML_INT int _func_xpath_new_node(roxml_parser_item_t *parser, char *chunk,
				   void *data)
{
	int cur = 0;
	...
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	if (!ctx->quoted && !ctx->dquoted && !ctx->parenthesys &&
	    !ctx->bracket) {
		int offset = 0;
		...
		ctx->new_node =
			roxml_set_axes(ctx->new_node, chunk + cur, &offset);
		cur = offset + 1;
	}
	ctx->shorten_cond = 0;
	return cur;
...
ROXML_INT int _func_xpath_quote(roxml_parser_item_t *parser, char *chunk,
				void *data)
{
	roxml_xpath_ctx_t *ctx = (roxml_xpath_ctx_t *)data;
#ifdef DEBUG_PARSING
...
ROXML_INT int _func_xpath_dquote(roxml_parser_item_t *parser, char *chunk,
				 void *data)
{
	roxml_xpath_ctx_t *ctx = (roxml_xpath_ctx_t *)data;
#ifdef DEBUG_PARSING
	...
	if (!ctx->quoted && !ctx->dquoted) {
		...
		if (ctx->new_cond) {
			if (ctx->new_cond->func == ROXML_FUNC_XPATH) {
				xpath_node_t *xp;
				ctx->new_cond->func2 = roxml_parse_xpath(
					ctx->new_cond->arg1, &xp, 1);
				ctx->new_cond->xp = xp;
			}
		} else {
			return -1;
...
ROXML_INT int _func_xpath_condition_or(roxml_parser_item_t *parser, char *chunk,
				       void *data)
{
	xpath_node_t *tmp_node;
	roxml_xpath_ctx_t *ctx = (roxml_xpath_ctx_t *)data;
	...
	if (strncmp(chunk, ROXML_COND_OR, len) == 0) {
		if (roxml_is_separator(*(chunk - 1)) &&
			...
			if (!ctx->bracket && !ctx->quoted && !ctx->dquoted) {
				...
				tmp_node = calloc(ctx->nbpath + 1,
						  sizeof(xpath_node_t));
				memcpy(tmp_node, ctx->first_node,
				       ctx->nbpath * sizeof(xpath_node_t));
				free(ctx->first_node);
				...
				ctx->nbpath++;
			} else if (ctx->bracket && !ctx->quoted &&
				   !ctx->dquoted) {
				if (ctx->new_cond->func != ROXML_FUNC_XPATH) {
					chunk[-1] = '\0';
					cur += strlen(ROXML_COND_OR);
					tmp_cond =
						calloc(1, sizeof(xpath_cond_t));
					if (ctx->new_cond) {
...
ROXML_INT int _func_xpath_path_or(roxml_parser_item_t *parser, char *chunk,
				  void *data)
{
	roxml_xpath_ctx_t *ctx = (roxml_xpath_ctx_t *)data;
	int cur = 0;
...
ROXML_INT int _func_xpath_operators(roxml_parser_item_t *parser, char *chunk,
				    void *data, int operator, int operator_bis)
{
	roxml_xpath_ctx_t *ctx = (roxml_xpath_ctx_t *)data;
	int cur = 0;
	if (!ctx->bracket && !ctx->quoted && !ctx->dquoted) {
					 ...
					 char *chunk, void *data)
{
	...
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_operators(parser, chunk, data, ROXML_OPERATOR_EQU,
				     ROXML_OPERATOR_EQU);
}

ROXML_INT int _func_xpath_operator_sup(roxml_parser_item_t *parser, char *chunk,
				       void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_operators(parser, chunk, data, ROXML_OPERATOR_SUP,
				     ROXML_OPERATOR_ESUP);
}

ROXML_INT int _func_xpath_operator_inf(roxml_parser_item_t *parser, char *chunk,
				       void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_operators(parser, chunk, data, ROXML_OPERATOR_INF,
				     ROXML_OPERATOR_EINF);
}
					...
					char *chunk, void *data)
{
	...
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_operators(parser, chunk, data, ROXML_OPERATOR_DIFF,
				     ROXML_OPERATOR_DIFF);
}

ROXML_INT int _func_xpath_number(roxml_parser_item_t *parser, char *chunk,
				 void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
...
ROXML_INT int _func_xpath_funcs(roxml_parser_item_t *parser, char *chunk,
				void *data, int func, char *name)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
...
ROXML_INT int _func_xpath_position(roxml_parser_item_t *parser, char *chunk,
				   void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_funcs(parser, chunk, data, ROXML_FUNC_POS,
				 ROXML_FUNC_POS_STR);
}

ROXML_INT int _func_xpath_first(roxml_parser_item_t *parser, char *chunk,
				void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_funcs(parser, chunk, data, ROXML_FUNC_FIRST,
				 ROXML_FUNC_FIRST_STR);
}

ROXML_INT int _func_xpath_last(roxml_parser_item_t *parser, char *chunk,
			       void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_funcs(parser, chunk, data, ROXML_FUNC_LAST,
				 ROXML_FUNC_LAST_STR);
}

ROXML_INT int _func_xpath_nsuri(roxml_parser_item_t *parser, char *chunk,
				void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_funcs(parser, chunk, data, ROXML_FUNC_NSURI,
				 ROXML_FUNC_NSURI_STR);
}

ROXML_INT int _func_xpath_lname(roxml_parser_item_t *parser, char *chunk,
				void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
#endif /* DEBUG_PARSING */
	return _func_xpath_funcs(parser, chunk, data, ROXML_FUNC_LNAME,
				 ROXML_FUNC_LNAME_STR);
}

ROXML_INT int _func_xpath_operator_add(roxml_parser_item_t *parser, char *chunk,
				       void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
...
ROXML_INT int _func_xpath_default(roxml_parser_item_t *parser, char *chunk,
				  void *data)
{
#ifdef DEBUG_PARSING
	fprintf(stderr, "calling func %s chunk %c\n", __func__, chunk[0]);
	...
	if ((ctx->is_first_node) || (ctx->wait_first_node)) {
		if (!ctx->quoted && !ctx->dquoted && !ctx->parenthesys &&
			...
			ctx->is_first_node = 0;
			ctx->wait_first_node = 0;
			ctx->new_node = roxml_set_axes(ctx->new_node,
						       chunk + cur, &offset);
			cur += offset;
		}
	} else if (ctx->bracket && !ctx->quoted && !ctx->dquoted) {
