...
// BEGIN generated code
	...
	{ -(MBEDTLS_ERR_PKCS12_FEATURE_UNAVAILABLE),
	  "PKCS12_FEATURE_UNAVAILABLE" },
	{ -(MBEDTLS_ERR_PKCS12_PBE_INVALID_FORMAT),
	  "PKCS12_PBE_INVALID_FORMAT" },
	...
	{ -(MBEDTLS_ERR_SSL_NO_CLIENT_CERTIFICATE),
	  "SSL_NO_CLIENT_CERTIFICATE" },
	{ -(MBEDTLS_ERR_SSL_CERTIFICATE_TOO_LARGE),
	  "SSL_CERTIFICATE_TOO_LARGE" },
				 ...
				 int tab_len, char *buf, size_t buflen)
{
	...
	mbedtls_snprintf(buf + MBEDTLS_ERR_PREFIX_LEN,
			 buflen - MBEDTLS_ERR_PREFIX_LEN, "UNKNOWN (0x%04X)",
			 err);
	return strlen(buf);
}
...
void mbedtls_strerror(int ret, char *buf, size_t buflen)
{
	...
	if (got_hl) {
		...
		if (use_ret == -(MBEDTLS_ERR_SSL_FATAL_ALERT_MESSAGE)) {
			strncpy(buf, "MBEDTLS_ERR_SSL_FATAL_ALERT_MESSAGE",
				buflen);
			buf[buflen - 1] = 0;
			return;
		}
	...
	mbedtls_err_to_str(use_ret, mbedtls_low_level_error_tab,
			   ARRAY_SIZE(mbedtls_low_level_error_tab), buf,
			   buflen);
}
