...
#define SCB_SHCSR_SECUREFAULTENA_Msk \
	(1UL                         \
	 << SCB_SHCSR_SECUREFAULTENA_Pos) /*!< SCB SHCSR: SECUREFAULTENA Mask */
...
#define SCB_SHCSR_SECUREFAULTACT_Msk \
	(1UL                         \
	 << SCB_SHCSR_SECUREFAULTACT_Pos) /*!< SCB SHCSR: SECUREFAULTACT Mask */
...
__STATIC_INLINE uint32_t TZ_NVIC_GetPriority_NS(IRQn_Type IRQn)
{
	if ((int32_t)(IRQn) >= 0) {
		...
		return (((uint32_t)SCB_NS
				 ->SHPR[(((uint32_t)IRQn) & 0xFUL) - 4UL] >>
			 (8U - __NVIC_PRIO_BITS)));
	}
}
#endif /*  defined (__ARM_FEATURE_CMSE) &&(__ARM_FEATURE_CMSE == 3U) */
