			   ...
			   lfs1_off_t off, void *buffer, lfs1_size_t size)
{
	...
	while (size > 0) {
		if (pcache && block == pcache->block && off >= pcache->off &&
			...
			lfs1_size_t diff =
				lfs1_min(size, lfs1->cfg->prog_size -
						       (off - pcache->off));
		...
		if (block == rcache->block && off >= rcache->off &&
			...
			lfs1_size_t diff =
				lfs1_min(size, lfs1->cfg->read_size -
						       (off - rcache->off));
		...
		if (off % lfs1->cfg->read_size == 0 &&
		    size >= lfs1->cfg->read_size) {
			// bypass cache?
			lfs1_size_t diff = size - (size % lfs1->cfg->read_size);
			int err = lfs1->cfg->read(lfs1->cfg, block, off, data,
						  diff);
			if (err) {
				return err;
			}
			  ...
			  lfs1_off_t off, const void *buffer, lfs1_size_t size)
{
	...
	for (lfs1_off_t i = 0; i < size; i++) {
		uint8_t c;
		int err = lfs1_cache_read(lfs1, rcache, pcache, block, off + i,
					  &c, 1);
		if (err) {
			return err;
		}
	...
	while (size > 0) {
		if (block == pcache->block && off >= pcache->off &&
			...
			if (off % lfs1->cfg->prog_size == 0) {
				...
				int err =
					lfs1_cache_flush(lfs1, pcache, rcache);
				if (err) {
					return err;
				}
			}
		...
		if (off % lfs1->cfg->prog_size == 0 &&
			...
			int err = lfs1->cfg->prog(lfs1->cfg, block, off, data,
						  diff);
			if (err) {
				return err;
			}
			...
			void *buffer, lfs1_size_t size)
{
	...
	return lfs1_cache_read(lfs1, &lfs1->rcache, NULL, block, off, buffer,
			       size);
}
...
{
	return lfs1_cache_prog(lfs1, &lfs1->pcache, NULL, block, off, buffer,
			       size);
}
...
static int lfs1_bd_cmp(lfs1_t *lfs1, lfs1_block_t block, lfs1_off_t off,
		       const void *buffer, lfs1_size_t size)
{
	return lfs1_cache_cmp(lfs1, &lfs1->rcache, NULL, block, off, buffer,
			      size);
}
...
static int lfs1_alloc(lfs1_t *lfs1, lfs1_block_t *block)
{
	while (true) {
		while (lfs1->free.i != lfs1->free.size) {
			...
			if (!(lfs1->free.buffer[off / 32] &
			      (1U << (off % 32)))) {
				// found a free block
				*block = (lfs1->free.off + off) %
					 lfs1->cfg->block_count;
		...
		lfs1->free.off = (lfs1->free.off + lfs1->free.size) %
				 lfs1->cfg->block_count;
		lfs1->free.size =
			lfs1_min(lfs1->cfg->lookahead, lfs1->free.ack);
		lfs1->free.i = 0;
			   ...
			   const struct lfs1_region *regions, int count)
{
	...
	while (true) {
		if (true) {
			...
			err = lfs1_bd_prog(lfs1, dir->pair[0], 0, &dir->d,
					   sizeof(dir->d));
			...
			while (newoff < (0x7fffffff & dir->d.size) - 4) {
				if (i < count && regions[i].oldoff == oldoff) {
					lfs1_crc(&crc, regions[i].newdata,
						 regions[i].newlen);
					...
					uint8_t data;
					err = lfs1_bd_read(lfs1, oldpair[1],
							   oldoff, &data, 1);
					if (err) {
						return err;
					}
...
relocate:
		...
		if (lfs1_paircmp(oldpair, (const lfs1_block_t[2]){ 0, 1 }) ==
		    0) {
			LFS1_WARN("Superblock %" PRIu32
	...
	if (relocated) {
		...
		LFS1_DEBUG("Relocating %" PRIu32 " %" PRIu32 " to %" PRIu32
			   " %" PRIu32,
			   ...
			   const void *data)
{
	...
	int err = lfs1_dir_commit(
		lfs1, dir,
		(struct lfs1_region[]){ { entry->off, sizeof(entry->d),
			   ...
			   const void *data)
{
	...
	while (true) {
		if (dir->d.size + lfs1_entry_size(entry) <=
		    lfs1->cfg->block_size) {
			...
			int err = lfs1_dir_commit(
				lfs1, dir,
				(struct lfs1_region[]){ { entry->off, 0,
							  &entry->d,
							  sizeof(entry->d) },
							{ entry->off, 0, data,
							  entry->d.nlen } },
...
static int lfs1_dir_next(lfs1_t *lfs1, lfs1_dir_t *dir, lfs1_entry_t *entry)
{
	...
	int err = lfs1_bd_read(lfs1, dir->pair[0], dir->off, &entry->d,
			       sizeof(entry->d));
	lfs1_entry_fromle32(&entry->d);
	if (err) {
		return err;
			 ...
			 lfs1_off_t *off)
{
	...
	while (current > target) {
		lfs1_size_t skip = lfs1_min(lfs1_npw2(current - target + 1) - 1,
					    lfs1_ctz(current));

		int err = lfs1_cache_read(lfs1, rcache, pcache, head, 4 * skip,
					  &head, 4);
	...
	while (true) {
		...
		if (true) {
			...
			if (size != lfs1->cfg->block_size) {
				for (lfs1_off_t i = 0; i < size; i++) {
					uint8_t data;
					err = lfs1_cache_read(lfs1, rcache,
							      NULL, head, i,
							      &data, 1);
					if (err) {
						return err;
					...
					err = lfs1_cache_prog(lfs1, pcache,
							      rcache, nblock, i,
							      &data, 1);
					if (err) {
						if (err == LFS1_ERR_CORRUPT) {
			     ...
			     void *data)
{
	...
	while (true) {
		...
		int count = 2 - (index & 1);
		err = lfs1_cache_read(lfs1, rcache, pcache, head, 0, &heads,
				      count * 4);
...
int lfs1_file_open(lfs1_t *lfs1, lfs1_file_t *file, const char *path, int flags)
{
...
static int lfs1_file_flush(lfs1_t *lfs1, lfs1_file_t *file)
{
	...
	if (file->flags & LFS1_F_WRITING) {
		...
		while (file->pos < file->size) {
			...
			uint8_t data;
			lfs1_ssize_t res =
				lfs1_file_read(lfs1, &orig, &data, 1);
			if (res < 0) {
				return res;
			}
		...
		while (true) {
			int err = lfs1_cache_flush(lfs1, &file->cache,
						   &lfs1->rcache);
			if (err) {
				if (err == LFS1_ERR_CORRUPT) {
					goto relocate;
...
int lfs1_file_sync(lfs1_t *lfs1, lfs1_file_t *file)
{
	...
	if ((file->flags & LFS1_F_DIRTY) && !(file->flags & LFS1_F_ERRED) &&
		...
		err = lfs1_bd_read(lfs1, cwd.pair[0], entry.off, &entry.d,
				   sizeof(entry.d));
		lfs1_entry_fromle32(&entry.d);
		if (err) {
			return err;
			    ...
			    lfs1_size_t size)
{
	...
	while (nsize > 0) {
		...
		lfs1_size_t diff =
			lfs1_min(nsize, lfs1->cfg->block_size - file->off);
		int err = lfs1_cache_read(lfs1, &file->cache, NULL, file->block,
					  file->off, data, diff);
		if (err) {
			return err;
		}
			     ...
			     const void *buffer, lfs1_size_t size)
{
	...
	if (!(file->flags & LFS1_F_WRITING) && file->pos > file->size) {
		...
		while (file->pos < pos) {
			lfs1_ssize_t res =
				lfs1_file_write(lfs1, file, &(uint8_t){ 0 }, 1);
			if (res < 0) {
				return res;
			}
		}
	...
	while (nsize > 0) {
		...
		if (!(file->flags & LFS1_F_WRITING) ||
			...
			if (!(file->flags & LFS1_F_WRITING) && file->pos > 0) {
				...
				int err = lfs1_ctz_find(
					lfs1, &file->cache, NULL, file->head,
			...
			lfs1_alloc_ack(lfs1);
			int err = lfs1_ctz_extend(lfs1, &lfs1->rcache,
						  &file->cache, file->block,
						  file->pos, &file->block,
						  &file->off);
			if (err) {
				file->flags |= LFS1_F_ERRED;
		...
		lfs1_size_t diff =
			lfs1_min(nsize, lfs1->cfg->block_size - file->off);
		while (true) {
			int err = lfs1_cache_prog(lfs1, &file->cache,
						  &lfs1->rcache, file->block,
						  file->off, data, diff);
			if (err) {
				if (err == LFS1_ERR_CORRUPT) {
					goto relocate;
...
int lfs1_file_truncate(lfs1_t *lfs1, lfs1_file_t *file, lfs1_off_t size)
{
	...
	if (size < oldsize) {
		...
		err = lfs1_ctz_find(lfs1, &file->cache, NULL, file->head,
				    file->size, size, &file->head,
				    &(lfs1_off_t){ 0 });
		if (err) {
			return err;
		}
...
int lfs1_rename(lfs1_t *lfs1, const char *oldpath, const char *newpath)
{
	...
	int err = lfs1_dir_find(lfs1, &oldcwd, &oldentry,
				&(const char *){ oldpath });
	if (err) {
		return err;
	}
...
static int lfs1_init(lfs1_t *lfs1, const struct lfs1_config *cfg)
{
	...
	LFS1_ASSERT(
		4 * lfs1_npw2(0xffffffff / (lfs1->cfg->block_size - 2 * 4)) <=
		lfs1->cfg->block_size);
...
int lfs1_format(lfs1_t *lfs1, const struct lfs1_config *cfg)
{
	...
	if (true) {
		...
		lfs1->free.off = 0;
		lfs1->free.size =
			lfs1_min(lfs1->cfg->lookahead, lfs1->cfg->block_count);
		lfs1->free.i = 0;
		lfs1_alloc_ack(lfs1);
		...
		for (int i = 0; i < 2; i++) {
			err = lfs1_dir_commit(
				...
				(struct lfs1_region[]){
					{ sizeof(superdir.d),
					  sizeof(superblock.d), &superblock.d,
					  sizeof(superblock.d) } },
				1);
			if (err && err != LFS1_ERR_CORRUPT) {
				goto cleanup;
		...
		err = lfs1_dir_fetch(lfs1, &superdir,
				     (const lfs1_block_t[2]){ 0, 1 });
		if (err) {
			goto cleanup;
		}
...
int lfs1_mount(lfs1_t *lfs1, const struct lfs1_config *cfg)
{
	...
	if (true) {
		...
		if ((major_version != LFS1_DISK_VERSION_MAJOR ||
			...
			LFS1_ERROR("Invalid version %d.%d", major_version,
				   minor_version);
			err = LFS1_ERR_INVAL;
			goto cleanup;
		}
...
int lfs1_traverse(lfs1_t *lfs1, int (*cb)(void *, lfs1_block_t), void *data)
{
	...
	while (true) {
		...
		while (dir.off + sizeof(entry.d) <=
		       (0x7fffffff & dir.d.size) - 4) {
			err = lfs1_bd_read(lfs1, dir.pair[0], dir.off, &entry.d,
					   sizeof(entry.d));
			lfs1_entry_fromle32(&entry.d);
			if (err) {
				return err;
			...
			if ((0x70 & entry.d.type) == (0x70 & LFS1_TYPE_REG)) {
				err = lfs1_ctz_traverse(lfs1, &lfs1->rcache,
							NULL,
							entry.d.u.file.head,
	...
	for (lfs1_file_t *f = lfs1->files; f; f = f->next) {
		if (f->flags & LFS1_F_DIRTY) {
			int err = lfs1_ctz_traverse(lfs1, &lfs1->rcache,
						    &f->cache, f->head, f->size,
						    cb, data);
			if (err) {
				return err;
			}
...
int lfs1_deorphan(lfs1_t *lfs1)
{
	...
	for (lfs1_size_t i = 0; i < lfs1->cfg->block_count; i++) {
		...
		if (!(0x80000000 & pdir.d.size)) {
			...
			lfs1_entry_t entry;
			int res =
				lfs1_parent(lfs1, pdir.d.tail, &parent, &entry);
			if (res < 0) {
				return res;
			}
		...
		while (true) {
			...
			if (entry.d.type & 0x80) {
				...
				if (moved) {
					LFS1_DEBUG("Found move %" PRIu32
						   " %" PRIu32,
						   entry.d.u.dir[0],
						   entry.d.u.dir[1]);
					err = lfs1_dir_remove(lfs1, &cwd,
							      &entry);
					if (err) {
						return err;
					}
				} else {
					LFS1_DEBUG("Found partial move %" PRIu32
						   " %" PRIu32,
						   entry.d.u.dir[0],
						   entry.d.u.dir[1]);
					entry.d.type &= ~0x80;
					err = lfs1_dir_update(lfs1, &cwd,
							      &entry, NULL);
					if (err) {
						return err;
					}
