			...
			lfs2_size_t size)
{
	...
	while (size > 0) {
		...
		if (pcache && block == pcache->block &&
			...
			if (off >= pcache->off) {
				...
				diff = lfs2_min(diff,
						pcache->size -
							(off - pcache->off));
				memcpy(data, &pcache->buffer[off - pcache->off],
				       diff);
		...
		if (block == rcache->block &&
			...
			if (off >= rcache->off) {
				...
				diff = lfs2_min(diff,
						rcache->size -
							(off - rcache->off));
				memcpy(data, &rcache->buffer[off - rcache->off],
				       diff);
		...
		if (size >= hint && off % lfs2->cfg->read_size == 0 &&
			...
			int err = lfs2->cfg->read(lfs2->cfg, block, off, data,
						  diff);
			if (err) {
				return err;
			}
		       ...
		       lfs2_size_t size)
{
	...
	for (lfs2_off_t i = 0; i < size; i += diff) {
		...
		int res = lfs2_bd_read(lfs2, pcache, rcache, hint - i, block,
				       off + i, &dat, diff);
		if (res) {
			return res;
		}
...
{
	if (pcache->block != LFS2_BLOCK_NULL &&
	    pcache->block != LFS2_BLOCK_INLINE) {
		LFS2_ASSERT(pcache->block < lfs2->cfg->block_count);
		lfs2_size_t diff =
			lfs2_alignup(pcache->size, lfs2->cfg->prog_size);
		...
		if (validate) {
			...
			int res = lfs2_bd_cmp(lfs2, NULL, rcache, diff,
					      pcache->block, pcache->off,
					      pcache->buffer, diff);
			if (res < 0) {
				return res;
			}
...
{
	const uint8_t *data = buffer;
	LFS2_ASSERT(block == LFS2_BLOCK_INLINE ||
		    block < lfs2->cfg->block_count);
	...
	while (size > 0) {
		if (block == pcache->block && off >= pcache->off &&
			...
			lfs2_size_t diff =
				lfs2_min(size, lfs2->cfg->cache_size -
						       (off - pcache->off));
			...
			pcache->size =
				lfs2_max(pcache->size, off - pcache->off);
			if (pcache->size == lfs2->cfg->cache_size) {
				// eagerly flush out pcache if we fill up
				int err = lfs2_bd_flush(lfs2, pcache, rcache,
							validate);
				if (err) {
					return err;
				}
...
#define LFS2_MKTAG(type, id, size)                               \
	(((lfs2_tag_t)(type) << 20) | ((lfs2_tag_t)(id) << 10) | \
	 (lfs2_tag_t)(size))
...
#define LFS2_MKATTRS(...)                                      \
	(struct lfs2_mattr[]){ __VA_ARGS__ },                  \
		sizeof((struct lfs2_mattr[]){ __VA_ARGS__ }) / \
			sizeof(struct lfs2_mattr)
...
static bool lfs2_mlist_isopen(struct lfs2_mlist *head, struct lfs2_mlist *node)
{
...
static int lfs2_alloc(lfs2_t *lfs2, lfs2_block_t *block)
{
	while (true) {
		while (lfs2->free.i != lfs2->free.size) {
			...
			if (!(lfs2->free.buffer[off / 32] &
			      (1U << (off % 32)))) {
				// found a free block
				*block = (lfs2->free.off + off) %
					 lfs2->cfg->block_count;
		...
		lfs2->free.off = (lfs2->free.off + lfs2->free.size) %
				 lfs2->cfg->block_count;
		lfs2->free.size =
			lfs2_min(8 * lfs2->cfg->lookahead_size, lfs2->free.ack);
		lfs2->free.i = 0;

		// find mask of free blocks from tree
		memset(lfs2->free.buffer, 0, lfs2->cfg->lookahead_size);
		int err = lfs2_fs_rawtraverse(lfs2, lfs2_alloc_lookahead, lfs2,
					      true);
		if (err) {
				     ...
				     lfs2_size_t gsize)
{
	...
	while (off >= sizeof(lfs2_tag_t) + lfs2_tag_dsize(ntag)) {
		...
		if ((gmask & tag) == (gmask & (gtag - gdiff))) {
			...
			err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, diff,
					   dir->pair[0],
					   off + sizeof(tag) + goff, gbuffer,
					   diff);
			if (err) {
				return err;
			}
	...
	while (true) {
		{
			if (off + lfs2_tag_dsize(ptag) < dir->off) {
				...
				int err = lfs2_bd_read(
					lfs2, NULL, &lfs2->rcache, sizeof(tag),
...
popped:
		...
		if (lfs2_tag_type3(tag) == LFS2_FROM_NOOP) {
			...
			for (unsigned i = 0; i < lfs2_tag_size(tag); i++) {
				...
				res = cb(data,
					 LFS2_MKTAG(LFS2_TYPE_USERATTR +
							    a[i].type,
						    lfs2_tag_id(tag) + diff,
						    a[i].size),
					 a[i].buffer);
				if (res < 0) {
					return res;
		    ...
		    void *data)
{
	...
	int r = 0;
	for (int i = 0; i < 2; i++) {
		int err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache,
				       sizeof(revs[i]), pair[i], 0, &revs[i],
				       sizeof(revs[i]));
	...
	for (int i = 0; i < 2; i++) {
		...
		uint32_t crc =
			lfs2_crc(0xffffffff, &dir->rev, sizeof(dir->rev));
		...
		while (true) {
			...
			if (!lfs2_tag_isvalid(tag)) {
				...
				break;
			} else if (off + lfs2_tag_dsize(tag) >
				   lfs2->cfg->block_size) {
				dir->erased = false;
				break;
			}
			...
			if (lfs2_tag_type1(tag) == LFS2_TYPE_CRC) {
				// check the crc attr
				uint32_t dcrc;
				err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache,
						   lfs2->cfg->block_size,
						   dir->pair[0],
						   off + sizeof(tag), &dcrc,
						   sizeof(dcrc));
				...
				ptag ^= (lfs2_tag_t)(lfs2_tag_chunk(tag) & 1U)
					<< 31;
				...
				lfs2->seed =
					lfs2_crc(lfs2->seed, &crc, sizeof(crc));
			...
			for (lfs2_off_t j = sizeof(tag);
			     j < lfs2_tag_dsize(tag); j++) {
				uint8_t dat;
				err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache,
						   lfs2->cfg->block_size,
						   dir->pair[0], off + j, &dat,
						   1);
				if (err) {
					if (err == LFS2_ERR_CORRUPT) {
						dir->erased = false;
			...
			if (lfs2_tag_type1(tag) == LFS2_TYPE_NAME) {
				...
				if (tag ==
					...
					tempbesttag |= 0x80000000;
					   ...
					   lfs2_tag_id(tag) <=
						   lfs2_tag_id(tempbesttag)) {
					tempbesttag += LFS2_MKTAG(
						0, lfs2_tag_splice(tag), 0);
				}
			...
			if ((fmask & tag) == (fmask & ftag)) {
				...
				if (res == LFS2_CMP_EQ) {
					// found a match
					tempbesttag = tag;
				} else if ((LFS2_MKTAG(0x7ff, 0x3ff, 0) &
					    tag) ==
					   (LFS2_MKTAG(0x7ff, 0x3ff, 0) &
					    tempbesttag)) {
		...
		if (dir->off > 0) {
			...
			if (id) {
				*id = lfs2_min(lfs2_tag_id(besttag),
					       dir->count);
			}
...
static int lfs2_dir_find_match(void *data, lfs2_tag_t tag, const void *buffer)
{
	...
	int res = lfs2_bd_cmp(lfs2, NULL, &lfs2->rcache, diff, disk->block,
			      disk->off, name->name, diff);
	if (res != LFS2_CMP_EQ) {
		return res;
	}
	...
	if (name->size != lfs2_tag_size(tag)) {
		return (name->size < lfs2_tag_size(tag)) ? LFS2_CMP_LT :
							   LFS2_CMP_GT;
	}
		...
		*path = name;
		...
		if (lfs2_tag_id(tag) != 0x3ff) {
			lfs2_stag_t res = lfs2_dir_get(
				...
				LFS2_MKTAG(LFS2_TYPE_STRUCT, lfs2_tag_id(tag),
					   8),
				dir->tail);
			if (res < 0) {
				return res;
		...
		while (true) {
			tag = lfs2_dir_fetchmatch(
				...
				lfs2_dir_find_match,
				&(struct lfs2_dir_find_match){ lfs2, name,
							       namelen });
			if (tag < 0) {
				return tag;
			}
			       ...
			       lfs2_tag_t tag, const void *buffer)
{
	...
	if (!(tag & 0x80000000)) {
		// from memory
		err = lfs2_dir_commitprog(lfs2, commit, buffer,
					  dsize - sizeof(tag));
		if (err) {
			return err;
		}
...
static int lfs2_dir_commitcrc(lfs2_t *lfs2, struct lfs2_commit *commit)
{
	...
	while (commit->off < end) {
		...
		commit->crc =
			lfs2_crc(commit->crc, &footer[0], sizeof(footer[0]));
...
static int lfs2_dir_alloc(lfs2_t *lfs2, lfs2_mdir_t *dir)
{
	...
	if (lfs2->cfg->block_cycles > 0) {
		dir->rev = lfs2_alignup(dir->rev,
					((lfs2->cfg->block_cycles + 1) | 1));
	}
			  ...
			  lfs2_mdir_t *source, uint16_t split, uint16_t end)
{
	...
	int res = lfs2_dir_compact(lfs2, &tail, attrs, attrcount, source, split,
				   end);
	if (res < 0) {
		return res;
	}
	...
	while (true) {
		{
			...
			err = lfs2_dir_traverse(
				lfs2, source, 0, 0xffffffff, attrs, attrcount,
				LFS2_MKTAG(0x400, 0x3ff, 0),
				LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), begin, end,
				-begin, lfs2_dir_commit_commit,
				&(struct lfs2_dir_commit_commit){ lfs2,
								  &commit });
			if (err) {
				if (err == LFS2_ERR_CORRUPT) {
					goto relocate;
			...
			if (!lfs2_pair_isnull(dir->tail)) {
				...
				err = lfs2_dir_commitattr(
					lfs2, &commit,
					LFS2_MKTAG(LFS2_TYPE_TAIL + dir->split,
						   0x3ff, 8),
					dir->tail);
				lfs2_pair_fromle32(dir->tail);
				if (err) {
...
relocate:
		...
		if (lfs2_pair_cmp(dir->pair, (const lfs2_block_t[2]){ 0, 1 }) ==
		    0) {
			LFS2_WARN("Superblock 0x%" PRIx32
...
static int lfs2_dir_splittingcompact(lfs2_t *lfs2, lfs2_mdir_t *dir,
				     const struct lfs2_mattr *attrs,
				     int attrcount, lfs2_mdir_t *source,
				     uint16_t begin, uint16_t end)
{
	while (true) {
		...
		while (end - split > 1) {
			lfs2_size_t size = 0;
			int err = lfs2_dir_traverse(
				lfs2, source, 0, 0xffffffff, attrs, attrcount,
				LFS2_MKTAG(0x400, 0x3ff, 0),
				LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), split, end,
				-split, lfs2_dir_commit_size, &size);
			if (err) {
				return err;
			}
	...
	if (lfs2_dir_needsrelocation(lfs2, dir) &&
		...
		if ((lfs2_size_t)size < lfs2->cfg->block_count / 2) {
			LFS2_DEBUG("Expanding superblock at rev %" PRIu32,
				   dir->rev);
	...
	return lfs2_dir_compact(lfs2, dir, attrs, attrcount, source, begin,
				end);
}
#endif
...
static int lfs2_dir_relocatingcommit(lfs2_t *lfs2, lfs2_mdir_t *dir,
				     const lfs2_block_t pair[2],
				     const struct lfs2_mattr *attrs,
				     int attrcount, lfs2_mdir_t *pdir)
{
	int state = 0;
	...
	if (dir->erased) {
		...
		lfs2_pair_tole32(dir->tail);
		int err = lfs2_dir_traverse(
			lfs2, dir, dir->off, dir->etag, attrs, attrcount, 0, 0,
			0, 0, 0, lfs2_dir_commit_commit,
...
compact:
	...
	state = lfs2_dir_splittingcompact(lfs2, dir, attrs, attrcount, dir, 0,
					  dir->count);
	if (state < 0) {
		return state;
	}
...
fixmlist:;
	...
	for (struct lfs2_mlist *d = lfs2->mlist; d; d = d->next) {
		if (lfs2_pair_cmp(d->m.pair, oldpair) == 0) {
			...
			if (d->m.pair != pair) {
				for (int i = 0; i < attrcount; i++) {
					if (lfs2_tag_type3(attrs[i].tag) ==
						    LFS2_TYPE_DELETE &&
					    d->id ==
						    lfs2_tag_id(attrs[i].tag)) {
						d->m.pair[0] = LFS2_BLOCK_NULL;
						d->m.pair[1] = LFS2_BLOCK_NULL;
					} else if (lfs2_tag_type3(
							   attrs[i].tag) ==
							   LFS2_TYPE_DELETE &&
						   d->id > lfs2_tag_id(
								   attrs[i].tag)) {
			...
			while (d->id >= d->m.count && d->m.split) {
				...
				d->id -= d->m.count;
				int err =
					lfs2_dir_fetch(lfs2, &d->m, d->m.tail);
				if (err) {
					return err;
				}
			}
				    ...
				    int attrcount)
{
	...
	lfs2_mdir_t pdir;
	int state = lfs2_dir_relocatingcommit(lfs2, &ldir, dir->pair, attrs,
					      attrcount, &pdir);
	if (state < 0) {
		return state;
	}
	...
	if (state == LFS2_OK_DROPPED) {
		...
		lpair[1] = pdir.pair[1];
		lfs2_pair_tole32(dir->tail);
		state = lfs2_dir_relocatingcommit(
			lfs2, &pdir, lpair,
			LFS2_MKATTRS({ LFS2_MKTAG(LFS2_TYPE_TAIL + dir->split,
						  0x3ff, 8),
				       dir->tail }),
			NULL);
		lfs2_pair_fromle32(dir->tail);
	...
	while (state == LFS2_OK_RELOCATED) {
		...
		for (struct lfs2_mlist *d = lfs2->mlist; d; d = d->next) {
			...
			if (d->type == LFS2_TYPE_DIR &&
			    lfs2_pair_cmp(lpair, ((lfs2_dir_t *)d)->head) ==
				    0) {
		...
		if (tag != LFS2_ERR_NOENT) {
			...
			state = lfs2_dir_relocatingcommit(
				lfs2, &pdir, ppair,
				LFS2_MKATTRS({ LFS2_MKTAG_IF(moveid != 0x3ff,
							     LFS2_TYPE_DELETE,
							     moveid, 0),
					       NULL },
					     { tag, ldir.pair }),
...
static int lfs2_rawmkdir(lfs2_t *lfs2, const char *path)
{
	...
	err = lfs2_dir_commit(
		lfs2, &dir,
		LFS2_MKATTRS({ LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8),
			       pred.tail }));
	...
	err = lfs2_dir_commit(
		...
		LFS2_MKATTRS({ LFS2_MKTAG(LFS2_TYPE_CREATE, id, 0), NULL },
			     ...
			     { LFS2_MKTAG(LFS2_TYPE_DIRSTRUCT, id, 8),
			       dir.pair },
			     { LFS2_MKTAG_IF(!cwd.m.split, LFS2_TYPE_SOFTTAIL,
					     0x3ff, 8),
			       dir.pair }));
	lfs2_pair_fromle32(dir.pair);
	if (err) {
...
static int lfs2_dir_rawopen(lfs2_t *lfs2, lfs2_dir_t *dir, const char *path)
{
	...
	if (lfs2_tag_id(tag) == 0x3ff) {
		...
		lfs2_stag_t res = lfs2_dir_get(
			lfs2, &dir->m, LFS2_MKTAG(0x700, 0x3ff, 0),
...

		{
			...
			if (noff != lfs2->cfg->block_size) {
				for (lfs2_off_t i = 0; i < noff; i++) {
					uint8_t data;
					err = lfs2_bd_read(lfs2, NULL, rcache,
							   noff - i, head, i,
							   &data, 1);
					if (err) {
						return err;
					...
					err = lfs2_bd_prog(lfs2, pcache, rcache,
							   true, nblock, i,
							   &data, 1);
					if (err) {
						if (err == LFS2_ERR_CORRUPT) {
			     ...
			     void *data)
{
	...
	while (true) {
		...
		int count = 2 - (index & 1);
		err = lfs2_bd_read(lfs2, pcache, rcache, count * sizeof(head),
				...
				const struct lfs2_file_config *cfg)
{
	...
	if (tag == LFS2_ERR_NOENT) {
		...
		err = lfs2_dir_commit(
			...
			LFS2_MKATTRS(
				{ LFS2_MKTAG(LFS2_TYPE_CREATE, file->id, 0),
				  NULL },
				{ LFS2_MKTAG(LFS2_TYPE_REG, file->id, nlen),
				  path },
				{ LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, file->id,
					     0),
				  NULL }));
		...
		tag = lfs2_dir_get(lfs2, &file->m, LFS2_MKTAG(0x700, 0x3ff, 0),
				   LFS2_MKTAG(LFS2_TYPE_STRUCT, file->id, 8),
				   &file->ctz);
		if (tag < 0) {
			err = tag;
			goto cleanup;
	...
	for (unsigned i = 0; i < file->cfg->attr_count; i++) {
		...
		if ((file->flags & LFS2_O_RDONLY) == LFS2_O_RDONLY) {
			lfs2_stag_t res = lfs2_dir_get(
				lfs2, &file->m, LFS2_MKTAG(0x7ff, 0x3ff, 0),
				LFS2_MKTAG(LFS2_TYPE_USERATTR +
	...
	if (lfs2_tag_type3(tag) == LFS2_TYPE_INLINESTRUCT) {
		...
		if (file->ctz.size > 0) {
			lfs2_stag_t res = lfs2_dir_get(
				lfs2, &file->m, LFS2_MKTAG(0x700, 0x3ff, 0),
...
static int lfs2_file_relocate(lfs2_t *lfs2, lfs2_file_t *file)
{
	while (true) {
		...
		for (lfs2_off_t i = 0; i < file->off; i++) {
			...
			if (file->flags & LFS2_F_INLINE) {
				err = lfs2_dir_getread(
					...
					LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT,
						   file->id, 0),
					i, &data, 1);
				if (err) {
					return err;
				...
				err = lfs2_bd_read(lfs2, &file->cache,
						   &lfs2->rcache, file->off - i,
						   file->block, i, &data, 1);
				if (err) {
					return err;
				}
			...
			err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache,
					   true, nblock, i, &data, 1);
			if (err) {
				if (err == LFS2_ERR_CORRUPT) {
					goto relocate;
				}
		...
		memcpy(file->cache.buffer, lfs2->pcache.buffer,
		       lfs2->cfg->cache_size);
	...
	if (file->flags & LFS2_F_WRITING) {
		...
		if (!(file->flags & LFS2_F_INLINE)) {
			...
			while (file->pos < file->ctz.size) {
				...
				uint8_t data;
				lfs2_ssize_t res = lfs2_file_flushedread(
					lfs2, &orig, &data, 1);
				if (res < 0) {
					return res;
				}

				res = lfs2_file_flushedwrite(lfs2, file, &data,
							     1);
				if (res < 0) {
					return res;
				}
			...
			while (true) {
				int err = lfs2_bd_flush(lfs2, &file->cache,
							&lfs2->rcache, true);
				if (err) {
					if (err == LFS2_ERR_CORRUPT) {
						goto relocate;
...
relocate:
				LFS2_DEBUG("Bad block at 0x%" PRIx32,
					   file->block);
...
static int lfs2_file_rawsync(lfs2_t *lfs2, lfs2_file_t *file)
{
	...
	if ((file->flags & LFS2_F_DIRTY) && !lfs2_pair_isnull(file->m.pair)) {
		...
		err = lfs2_dir_commit(
			lfs2, &file->m,
			LFS2_MKATTRS({ LFS2_MKTAG(type, file->id, size),
				       buffer },
					  ...
					  void *buffer, lfs2_size_t size)
{
	...
	while (nsize > 0) {
		...
		if (!(file->flags & LFS2_F_READING) ||
			...
			if (!(file->flags & LFS2_F_INLINE)) {
				int err = lfs2_ctz_find(
					lfs2, NULL, &file->cache,
		...
		lfs2_size_t diff =
			lfs2_min(nsize, lfs2->cfg->block_size - file->off);
		if (file->flags & LFS2_F_INLINE) {
			int err = lfs2_dir_getread(
				lfs2, &file->m, NULL, &file->cache,
				lfs2->cfg->block_size,
				LFS2_MKTAG(0xfff, 0x1ff, 0),
			...
			int err = lfs2_bd_read(lfs2, NULL, &file->cache,
					       lfs2->cfg->block_size,
					       file->block, file->off, data,
					       diff);
			if (err) {
				return err;
			}
	...
	while (nsize > 0) {
		...
		if (!(file->flags & LFS2_F_WRITING) ||
			...
			if (!(file->flags & LFS2_F_INLINE)) {
				...
				lfs2_alloc_ack(lfs2);
				int err = lfs2_ctz_extend(
					lfs2, &file->cache, &lfs2->rcache,
		...
		lfs2_size_t diff =
			lfs2_min(nsize, lfs2->cfg->block_size - file->off);
		while (true) {
			int err = lfs2_bd_prog(lfs2, &file->cache,
					       &lfs2->rcache, true, file->block,
					       file->off, data, diff);
			if (err) {
				if (err == LFS2_ERR_CORRUPT) {
					goto relocate;
				       ...
				       const void *buffer, lfs2_size_t size)
{
	...
	if (!(file->flags & LFS2_F_WRITING) && file->pos > file->ctz.size) {
		...
		while (file->pos < pos) {
			lfs2_ssize_t res = lfs2_file_flushedwrite(
				lfs2, file, &(uint8_t){ 0 }, 1);
			if (res < 0) {
				return res;
			}
		}
				 ...
				 lfs2_off_t size)
{
	...
	if (size < oldsize) {
		...
		lfs2_soff_t res =
			lfs2_file_rawseek(lfs2, file, 0, LFS2_SEEK_END);
		if (res < 0) {
			return (int)res;
		}
		...
		// fill with zeros
		while (file->pos < size) {
			res = lfs2_file_rawwrite(lfs2, file, &(uint8_t){ 0 },
						 1);
			if (res < 0) {
				return (int)res;
			}
...
static int lfs2_rawremove(lfs2_t *lfs2, const char *path)
{
	...
	if (lfs2_tag_type3(tag) == LFS2_TYPE_DIR) {
		...
		lfs2_block_t pair[2];
		lfs2_stag_t res = lfs2_dir_get(
			lfs2, &cwd, LFS2_MKTAG(0x700, 0x3ff, 0),
			LFS2_MKTAG(LFS2_TYPE_STRUCT, lfs2_tag_id(tag), 8),
			pair);
		if (res < 0) {
			return (int)res;
	...
	err = lfs2_dir_commit(lfs2, &cwd,
			      LFS2_MKATTRS({ LFS2_MKTAG(LFS2_TYPE_DELETE,
							lfs2_tag_id(tag), 0),
					     NULL }));
	if (err) {
		lfs2->mlist = dir.next;
			  ...
			  const char *newpath)
{
	...
	if (prevtag == LFS2_ERR_NOENT) {
		...
		lfs2_stag_t res = lfs2_dir_get(
			lfs2, &newcwd, LFS2_MKTAG(0x700, 0x3ff, 0),
	...
	err = lfs2_dir_commit(
		lfs2, &newcwd,
		LFS2_MKATTRS({ LFS2_MKTAG_IF(prevtag != LFS2_ERR_NOENT,
					     LFS2_TYPE_DELETE, newid, 0),
			       NULL },
			     { LFS2_MKTAG(LFS2_TYPE_CREATE, newid, 0), NULL },
			     { LFS2_MKTAG(lfs2_tag_type3(oldtag), newid,
					  strlen(newpath)),
			       newpath },
			     { LFS2_MKTAG(LFS2_FROM_MOVE, newid,
					  lfs2_tag_id(oldtag)),
			       &oldcwd },
			     { LFS2_MKTAG_IF(samepair, LFS2_TYPE_DELETE,
					     newoldid, 0),
			       NULL }));
	if (err) {
			   ...
			   const void *buffer, lfs2_size_t size)
{
	...
	return lfs2_dir_commit(
		lfs2, &cwd,
		LFS2_MKATTRS({ LFS2_MKTAG(LFS2_TYPE_USERATTR + type, id, size),
			       buffer }));
}
#endif
...
static int lfs2_rawformat(lfs2_t *lfs2, const struct lfs2_config *cfg)
{
	int err = 0;
	{
		...
		err = lfs2_dir_commit(
			...
			LFS2_MKATTRS({ LFS2_MKTAG(LFS2_TYPE_CREATE, 0, 0),
				     ...
				     { LFS2_MKTAG(LFS2_TYPE_SUPERBLOCK, 0, 8),
				       "littlefs" },
				     { LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, 0,
						  sizeof(superblock)),
				       &superblock }));
		if (err) {
			goto cleanup;
		...
		err = lfs2_dir_fetch(lfs2, &root,
				     (const lfs2_block_t[2]){ 0, 1 });
		if (err) {
			goto cleanup;
		}
	}
...
static int lfs2_rawmount(lfs2_t *lfs2, const struct lfs2_config *cfg)
{
	...
	while (!lfs2_pair_isnull(dir.tail)) {
		...
		lfs2_stag_t tag = lfs2_dir_fetchmatch(
			...
			LFS2_MKTAG(LFS2_TYPE_SUPERBLOCK, 0, 8), NULL,
			lfs2_dir_find_match,
		...
		if (tag && !lfs2_tag_isdelete(tag)) {
			...
			uint16_t major_version =
				(0xffff & (superblock.version >> 16));
			uint16_t minor_version =
				(0xffff & (superblock.version >> 0));
			if ((major_version != LFS2_DISK_VERSION_MAJOR ||
			     minor_version > LFS2_DISK_VERSION_MINOR)) {
				LFS2_ERROR("Invalid version v%" PRIu16
					   ".%" PRIu16,
			...
			if (superblock.block_count != lfs2->cfg->block_count) {
				LFS2_ERROR("Invalid block count (%" PRIu32
					   " != %" PRIu32 ")",
			...
			if (superblock.block_size != lfs2->cfg->block_size) {
				LFS2_ERROR("Invalid block size (%" PRIu32
					   " != %" PRIu32 ")",
					   superblock.block_count,
					   lfs2->cfg->block_count);
				err = LFS2_ERR_INVAL;
	...
	if (!lfs2_gstate_iszero(&lfs2->gstate)) {
		LFS2_DEBUG("Found pending gstate 0x%08" PRIx32 "%08" PRIx32
			   "%08" PRIx32,
			   lfs2->gstate.tag, lfs2->gstate.pair[0],
			   lfs2->gstate.pair[1]);
	}
			...
			void *data, bool includeorphans)
{
	...
	while (!lfs2_pair_isnull(dir.tail)) {
		...
		for (uint16_t id = 0; id < dir.count; id++) {
			...
			lfs2_stag_t tag = lfs2_dir_get(
				lfs2, &dir, LFS2_MKTAG(0x700, 0x3ff, 0),
				LFS2_MKTAG(LFS2_TYPE_STRUCT, id, sizeof(ctz)),
				&ctz);
			if (tag < 0) {
				if (tag == LFS2_ERR_NOENT) {
					continue;
			...
			if (lfs2_tag_type3(tag) == LFS2_TYPE_CTZSTRUCT) {
				err = lfs2_ctz_traverse(lfs2, NULL,
							&lfs2->rcache, ctz.head,
							ctz.size, cb, data);
				if (err) {
					return err;
				}
	...
	for (lfs2_file_t *f = (lfs2_file_t *)lfs2->mlist; f; f = f->next) {
		...
		if ((f->flags & LFS2_F_DIRTY) && !(f->flags & LFS2_F_INLINE)) {
			int err = lfs2_ctz_traverse(lfs2, &f->cache,
						    &lfs2->rcache, f->ctz.head,
						    f->ctz.size, cb, data);
			if (err) {
				return err;
			}
...
static int lfs2_fs_parent_match(void *data, lfs2_tag_t tag, const void *buffer)
{
	...
	int err = lfs2_bd_read(lfs2, &lfs2->pcache, &lfs2->rcache,
			       lfs2->cfg->block_size, disk->block, disk->off,
			       &child, sizeof(child));
	if (err) {
		return err;
	}
	...
	return (lfs2_pair_cmp(child, find->pair) == 0) ? LFS2_CMP_EQ :
							 LFS2_CMP_LT;
}
#endif
				  ...
				  lfs2_mdir_t *parent)
{
	...
	while (!lfs2_pair_isnull(parent->tail)) {
		...
		lfs2_stag_t tag = lfs2_dir_fetchmatch(
			...
			LFS2_MKTAG(LFS2_TYPE_DIRSTRUCT, 0, 8), NULL,
			lfs2_fs_parent_match,
			&(struct lfs2_fs_parent_match){ lfs2,
							{ pair[0], pair[1] } });
...
static int lfs2_fs_preporphans(lfs2_t *lfs2, int8_t orphans)
{
	...
	lfs2->gstate.tag =
		((lfs2->gstate.tag & ~LFS2_MKTAG(0x800, 0, 0)) |
...
restart: {
	...
	while (!lfs2_pair_isnull(pdir.tail)) {
		...
		if (!pdir.split) {
			...
			lfs2_mdir_t parent;
			lfs2_stag_t tag =
				lfs2_fs_parent(lfs2, pdir.tail, &parent);
			if (tag < 0 && tag != LFS2_ERR_NOENT) {
				return tag;
			}
			...
			if (tag == LFS2_ERR_NOENT && powerloss) {
				// we are an orphan
				LFS2_DEBUG("Fixing orphan {0x%" PRIx32
					   ", 0x%" PRIx32 "}",
					   pdir.tail[0], pdir.tail[1]);

				// steal state
				err = lfs2_dir_getgstate(lfs2, &dir,
							 &lfs2->gdelta);
				if (err) {
					return err;
				}
				...
				// steal tail
				lfs2_pair_tole32(dir.tail);
				int state = lfs2_dir_orphaningcommit(
					lfs2, &pdir,
					LFS2_MKATTRS(
						{ LFS2_MKTAG(LFS2_TYPE_TAIL +
								     dir.split,
			...
			if (tag != LFS2_ERR_NOENT) {
				lfs2_block_t pair[2];
				lfs2_stag_t state = lfs2_dir_get(
					lfs2, &parent,
				...
				if (!lfs2_pair_sync(pair, pdir.tail)) {
					...
					uint16_t moveid = 0x3ff;
					if (lfs2_gstate_hasmovehere(
						    &lfs2->gstate, pdir.pair)) {
						moveid = lfs2_tag_id(
							lfs2->gstate.tag);
						LFS2_DEBUG(
							...
							pdir.pair[0],
							pdir.pair[1], moveid);
						lfs2_fs_prepmove(lfs2, 0x3ff,
								 NULL);
					}

					lfs2_pair_tole32(pair);
					state = lfs2_dir_orphaningcommit(
						lfs2, &pdir,
						LFS2_MKATTRS(
							{ LFS2_MKTAG_IF(
								  moveid !=
	...
	return lfs2_fs_preporphans(
		lfs2, -lfs2_min(lfs2_gstate_getorphans(&lfs2->gstate), found));
}
#endif
			 ...
			 void *buffer, lfs2_size_t size)
{
	...
	return lfs2_bd_read(lfs2, &lfs2->pcache, &lfs2->rcache, size, block,
			    off, buffer, size);
}
			   ...
			   const lfs2_block_t pair[2])
{
	...
	for (int i = 0; i < 2; i++) {
		...
		err = lfs21_bd_crc(lfs2, tpair[i], sizeof(test),
				   (0x7fffffff & test.size) - sizeof(test),
				   &crc);
		if (err) {
			if (err == LFS2_ERR_CORRUPT) {
				continue;
...
static int lfs21_dir_next(lfs2_t *lfs2, lfs21_dir_t *dir, lfs21_entry_t *entry)
{
	...
	int err = lfs21_bd_read(lfs2, dir->pair[0], dir->off, &entry->d,
				sizeof(entry->d));
...
int lfs21_traverse(lfs2_t *lfs2, int (*cb)(void *, lfs2_block_t), void *data)
{
	...
	while (true) {
		...
		while (dir.off + sizeof(entry.d) <=
		       (0x7fffffff & dir.d.size) - 4) {
		       ...
		       const struct lfs2_config *cfg)
{
	int err = 0;
	{
		...
		if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
			LFS2_ERROR("Invalid superblock at {0x%" PRIx32
				   ", 0x%" PRIx32 "}",
				   0, 1);
			err = LFS2_ERR_CORRUPT;
			goto cleanup;
		...
		if ((major_version != LFS21_DISK_VERSION_MAJOR ||
			...
			LFS2_ERROR("Invalid version v%d.%d", major_version,
				   minor_version);
			err = LFS2_ERR_INVAL;
			goto cleanup;
		}
...

	{
		...
		while (!lfs2_pair_isnull(dir1.d.tail)) {
			...
			LFS2_DEBUG("Migrating {0x%" PRIx32 ", 0x%" PRIx32 "} "
				   "-> {0x%" PRIx32 ", 0x%" PRIx32 "}",
				   lfs2->root[0], lfs2->root[1], dir1.head[0],
				   dir1.head[1]);
			...
			for (lfs2_off_t i = 0; i < dir2.off; i++) {
				uint8_t dat;
				err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache,
						   dir2.off, dir2.pair[0], i,
						   &dat, 1);
				if (err) {
					goto cleanup;
				}

				err = lfs2_bd_prog(lfs2, &lfs2->pcache,
						   &lfs2->rcache, true,
						   dir1.head[1], i, &dat, 1);
				if (err) {
					goto cleanup;
			...
			err = lfs2_bd_flush(lfs2, &lfs2->pcache, &lfs2->rcache,
					    true);
			if (err) {
				goto cleanup;
			}
...
int lfs2_format(lfs2_t *lfs2, const struct lfs2_config *cfg)
{
	...
	LFS2_TRACE("lfs2_format(%p, %p {.context=%p, "
		   ...
		   cfg->lookahead_buffer, cfg->name_max, cfg->file_max,
		   cfg->attr_max);
...
int lfs2_stat(lfs2_t *lfs2, const char *path, struct lfs2_info *info)
{
	...
	LFS2_TRACE("lfs2_stat(%p, \"%s\", %p)", (void *)lfs2, path,
		   (void *)info);
...
int lfs2_removeattr(lfs2_t *lfs2, const char *path, uint8_t type)
{
	...
	LFS2_TRACE("lfs2_removeattr(%p, \"%s\", %" PRIu8 ")", (void *)lfs2,
		   path, type);
...
int lfs2_file_open(lfs2_t *lfs2, lfs2_file_t *file, const char *path, int flags)
{
	...
	LFS2_TRACE("lfs2_file_open(%p, %p, \"%s\", %x)", (void *)lfs2,
		   (void *)file, path, flags);
		      ...
		      int flags, const struct lfs2_file_config *cfg)
{
	...
	LFS2_TRACE("lfs2_file_opencfg(%p, %p, \"%s\", %x, %p {"
		   ...
		   (void *)lfs2, (void *)file, path, flags, (void *)cfg,
...
int lfs2_dir_seek(lfs2_t *lfs2, lfs2_dir_t *dir, lfs2_off_t off)
{
	...
	LFS2_TRACE("lfs2_dir_seek(%p, %p, %" PRIu32 ")", (void *)lfs2,
		   (void *)dir, off);
