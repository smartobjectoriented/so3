 ...
 */
static void allocate_page(pcb_t *pcb, addr_t virt_addr, int nr_pages,
                          bool usr) {
        int i;
        addr_t page;
...
void *preserve_args_and_env(int argc, char **argv, char **envp) {
        ...
        /* First env. variable */
        __args =
            (char **) (args + sizeof(int) + sizeof(char *) * (*((int *) args)));
...
void post_setup_image(void *args_env) {
        ...
        for (i = 0; i < argc; i++)
                __args[i] =
                    ((addr_t) __args[i] - (addr_t) args_env) + args_base;
...
void load_process(elf_img_info_t *elf_img_info) {
        ...
        for (i = 0; i < elf_img_info->header->e_phnum; i++) {
                ...
                for (j = 0; j < elf_img_info->header->e_shnum; j++) {
                        ...
                        section_end =
                            section_start + elf_img_info->sections[j].sh_size;
...
#warning do_exec() must be completed...
        ...
        if (!list_empty(&current()->joinQueue)) {
                cur =
                    list_entry(current()->joinQueue.next, queue_thread_t, list);
...
int do_fork(void) {
        ...
        newp->main_thread =
            user_thread(NULL, newp->name, (void *) arch_get_args_base(), newp);

        /* Copy the kernel stack of the main thread */
        memcpy(
            (void *) get_kernel_stack_top(newp->main_thread->stack_slotID) -
                THREAD_STACK_SIZE,
...
void dump_proc(void) {
        ...
        list_for_each_entry(pcb, &proc_list, list) {
                ...
                printk(
                    " [pid %d state: %s] [main_tid: %d name: %s]\n", pcb->pid,
                    proc_state_str(pcb->state),
                    ((pcb->main_thread != NULL) ? pcb->main_thread->tid : -1),
