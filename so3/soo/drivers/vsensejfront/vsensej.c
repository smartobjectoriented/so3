...
{
	vsensej_priv_t *vsensej_priv =
		(vsensej_priv_t *)dev_get_drvdata(vsensej_dev->dev);
...
static void vsensej_probe(struct vbus_device *vdev)
{
	...
	vsensej_priv->vsensej.irq = bind_evtchn_to_irq_handler(
		evtchn, vsensej_interrupt, NULL, vdev);
	...
	vsensej_priv->vsensej.ring_ref = vbus_grant_ring(
		vdev, phys_to_pfn(virt_to_phys_pt(
			      (addr_t)vsensej_priv->vsensej.ring.sring)));

	vbus_transaction_start(&vbt);

	vbus_printf(vbt, vdev->nodename, "ring-ref", "%u",
		    vsensej_priv->vsensej.ring_ref);
	vbus_printf(vbt, vdev->nodename, "ring-evtchn", "%u",
		    vsensej_priv->vsensej.evtchn);
...
static void vsensej_reconfiguring(struct vbus_device *vdev)
{
	...
	FRONT_RING_INIT(&vsensej_priv->vsensej.ring,
			vsensej_priv->vsensej.ring.sring, PAGE_SIZE);
...
vdrvfront_t vsensejdrv = { .probe = vsensej_probe,
