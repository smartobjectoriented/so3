...
static int tx_buffer_put(uint8_t *data, uint32_t size, uint8_t type)
{
	...
	vuihandler_priv =
		(vuihandler_priv_t *)dev_get_drvdata(vuihandler_dev->dev);
	...
	memcpy(tx_circ_buf->circ_buf[tx_circ_buf->cur_prod_idx].data, data,
	       size);
	tx_circ_buf->circ_buf[tx_circ_buf->cur_prod_idx].type = type;

	/* Update the circular buffer info */
	tx_circ_buf->cur_prod_idx =
		(tx_circ_buf->cur_prod_idx + 1) % VUIHANDLER_MAX_TX_BUF_ENTRIES;
...
static tx_buf_entry_t *tx_buffer_get(void)
{
	...
	tx_circ_buf->cur_cons_idx =
		(tx_circ_buf->cur_cons_idx + 1) % VUIHANDLER_MAX_TX_BUF_ENTRIES;
	tx_circ_buf->cur_size--;
...
static void process_pending_tx_rsp(struct vbus_device *vdev)
{
	...
	while ((ring_req = vuihandler_tx_get_ring_response(
			&vuihandler->tx_ring)) != NULL)
		;
}
...
static void process_pending_rx_rsp(struct vbus_device *vdev)
{
	...
	while ((ring_rsp = vuihandler_rx_get_ring_response(
			&vuihandler->rx_ring)) != NULL) {
	...
	while (true) {
		...
		*/
		if (!RING_REQ_FULL(&vuihandler_priv->vuihandler.tx_ring)) {
			ring_req = vuihandler_tx_new_ring_request(
				&vuihandler_priv->vuihandler.tx_ring);
			...
			vuihandler_tx_ring_request_ready(
				&vuihandler_priv->vuihandler.tx_ring);

			notify_remote_via_virq(
				vuihandler_priv->vuihandler.tx_irq);
		}
...
void vuihandler_probe(struct vbus_device *vdev)
{
	...
	vuihandler_priv->vuihandler.rx_irq = bind_evtchn_to_irq_handler(
		rx_evtchn, vuihandler_rx_interrupt, NULL, vdev);
	...
	FRONT_RING_INIT(&vuihandler_priv->vuihandler.rx_ring, rx_sring,
			PAGE_SIZE);

	/* Prepare the shared to page to be visible on the other end */

	vuihandler_priv->vuihandler.rx_ring_ref = vbus_grant_ring(
		vdev,
		phys_to_pfn(virt_to_phys_pt(
			(addr_t)vuihandler_priv->vuihandler.rx_ring.sring)));

	vbus_transaction_start(&vbt);

	vbus_printf(vbt, vdev->nodename, "rx_ring-ref", "%u",
		    vuihandler_priv->vuihandler.rx_ring_ref);
	vbus_printf(vbt, vdev->nodename, "rx_ring-evtchn", "%u",
		    vuihandler_priv->vuihandler.rx_evtchn);
	...
	vuihandler_priv->vuihandler.tx_irq = bind_evtchn_to_irq_handler(
		tx_evtchn, vuihandler_tx_interrupt, NULL, vdev);
	...
	FRONT_RING_INIT(&vuihandler_priv->vuihandler.tx_ring, tx_sring,
			PAGE_SIZE);

	/* Prepare the shared to page to be visible on the other end */

	vuihandler_priv->vuihandler.tx_ring_ref = vbus_grant_ring(
		vdev,
		phys_to_pfn(virt_to_phys_pt(
			(addr_t)vuihandler_priv->vuihandler.tx_ring.sring)));

	vbus_transaction_start(&vbt);

	vbus_printf(vbt, vdev->nodename, "tx_ring-ref", "%u",
		    vuihandler_priv->vuihandler.tx_ring_ref);
	vbus_printf(vbt, vdev->nodename, "tx_ring-evtchn", "%u",
		    vuihandler_priv->vuihandler.tx_evtchn);
	...
	/* Start the TX thread */
	kernel_thread(vuihandler_send_fn, "vuihandler_send_fn", (void *)vdev,
		      0);
}
...
void vuihandler_reconfiguring(struct vbus_device *vdev)
{
	...
	FRONT_RING_INIT(&vuihandler_priv->vuihandler.rx_ring,
			(&vuihandler_priv->vuihandler.rx_ring)->sring,
			PAGE_SIZE);
	...
	FRONT_RING_INIT(&vuihandler_priv->vuihandler.tx_ring,
			(&vuihandler_priv->vuihandler.tx_ring)->sring,
			PAGE_SIZE);
