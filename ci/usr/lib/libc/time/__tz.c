...
static const unsigned char *zi, *trans, *__index, *types, *abbrevs, *abbrevs_end;
...
static void do_tzset()
{
	...
	if (map) {
		...
		__index = trans + (zi_read32(trans-12) << scale);
		types = __index + zi_read32(trans-12);
...
static size_t scan_trans(long long t, int local, size_t *alt)
{
	...
	size_t a = 0, n = (__index-trans)>>scale, m;
	...
	while (n > 1) {
		...
		if (local) off = (int32_t)zi_read32(types + 6 * __index[m-1]);
		if (t - off < (int64_t)x) {
			n /= 2;
		} else {
			a = m;
	...
	/* First and last entry are special. First means to use lowest-__index
	 * non-DST type. Last means to apply POSIX-style rule if available. */
	n = (__index-trans)>>scale;
	if (a == n-1) return -1;
	if (a == 0) {
		x = zi_read32(trans + (a<<scale));
		if (scale == 3) x = x<<32 | zi_read32(trans + (a<<scale) + 4);
		else x = (int32_t)x;
		if (local) off = (int32_t)zi_read32(types + 6 * __index[a-1]);
		if (t - off < (int64_t)x) {
	...
	if (alt) {
		if (a && types[6*__index[a-1]+4] != types[6*__index[a]+4])
			*alt = __index[a-1];
		else if (a+1<n && types[6*__index[a+1]+4] != types[6*__index[a]+4])
			*alt = __index[a+1];
		else
			*alt = __index[a];
	}
