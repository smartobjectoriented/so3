...
p.label(780, 385, 270, 120, "SO3 capsules (S3C) run as\nlightweight EL1 guests beside a\nLinux agency.\n\nThe agency is built here via the\n:soo override; its SOO patches\nderive from a separate project.", 9)
pages.append(p)
...
fit = p.box(280, 90, 190, 64, "FIT image(s) (.itb)\nrendered from the BSP\nlayer (files/its/)", NEUTRAL, 10)
...
p.box(670, 120, 160, 60, "EL0 — guest\nuser space", USER, 10)
p.box(850, 120, 160, 60, "EL0 — capsule\nuser space", CAPS, 10)
p.box(670, 200, 160, 60, "EL1 — guest\nSO3 kernel", USER, 10)
...
ag = p.box(100, 120, 200, 150, "Agency  (DOMID_AGENCY)\nLinux — owns the devices\nvbstore server", USER, 10)
...
itb = p.box(70, 420, 250, 64,
            "do_itb → render + mkimage\nfiles/its/*.its  →  images/*.itb", WHITE, 10)
